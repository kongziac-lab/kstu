...
// 교육부 대시보드 전용 — 연도별로 실제 발표된 우수인증 명단(교육부·법무부 공동
// "교육국제화역량 인증대학" 보도자료, 매년 2월 발표분을 4/1 KEDI 스냅샷에 대응시킴).
// 2013~2022년은 원문(보도자료 첨부 hwp/PDF)을 찾지 못해 데이터가 없다 — 이
// 인증제는 등락이 있어(예: 고려대는 2023년 우수인증 → 2024년 탈락 → 2025년
// 재인증) 모르는 연도에 인접 연도 값으로 채우면 오히려 틀린 정보가 된다.
// 일반인증은 매년 총 개수만 보도되고 실명 명단을 확보하지 못해 어느 연도도
// 지원하지 않는다 — 교육부 대시보드에는 우수인증 배지만 있고, 그마저도
// 2023~2025년에만 표시된다.
const MOE_EXCELLENT_CERTIFIED_BY_YEAR = {
  // 출처: 교육부·법무부 공동 보도자료(2023.03.03 studyinkoreanews.com 재보도), 13개교
  2023: `건국대학교, 고려대학교, 서경대학교, 서울시립대학교, 성신여자대학교, 울산과학기술원, 이화여자대학교, 한양대학교, 홍익대학교, 국립암센터국제암대학원, 한국개발연구원국제정책대학원, 한국전력국제원자력대학원`,
  // 출처: 교육부 보도자료(2024.02.07, korea.kr 156614628) / 베리타스알파(2024.03.12, idxno=496925), 15개교
  2024: `건국대학교, 경북대학교, 계명대학교, 덕성여자대학교, 동국대학교, 부산대학교, 서경대학교, 서울시립대학교, 서울신학대학교, 성신여자대학교, 이화여자대학교, 중앙대학교, 포항공과대학교, 한양대학교, 홍익대학교`,
  // 출처: 교육부·법무부 공동 보도자료(2025.02.24~25, 베리타스알파 idxno=543405 등 5개 매체 교차확인), 27개교
  // = 2024년 15개교 + 신규 7개교(고려대·선문대·성균관대·세명대·숙명여대·충남대·한성대)
  //   + 경복대(전문대) + 개신대학원대·과학기술연합대학원대(UST)·온석대학원대·KDI국제정책대학원
  2025: `건국대학교, 경북대학교, 계명대학교, 덕성여자대학교, 동국대학교, 부산대학교, 서경대학교, 서울시립대학교, 서울신학대학교, 성신여자대학교, 이화여자대학교, 중앙대학교, 포항공과대학교, 한양대학교, 홍익대학교, 고려대학교, 선문대학교, 성균관대학교, 세명대학교, 숙명여자대학교, 충남대학교, 한성대학교, 경복대학교, 개신대학원대학교, 과학기술연합대학원대학교, 온석대학원대학교, 한국개발연구원국제정책대학원`,
};

const moeExcellentCertifiedSetByYear = new Map(
  Object.entries(MOE_EXCELLENT_CERTIFIED_BY_YEAR).map(([year, list]) => [
    Number(year),
    new Set(list.split(",").map(normalizeSchoolName).filter(Boolean)),
  ])
);

/**
 * 교육부 대시보드 전용 인증 판정. 실제 발표 원문을 확인한 2023~2025년만 판정하고,
 * 그 외 연도는 항상 null(배지 미표시)을 반환한다 — 법무부용 getCertification()과
 * 달리 "현재 명단을 과거에도 적용"하는 근사치를 쓰지 않는다.
 * @param {string} name
 * @param {number} year
 */
export function getMoeCertification(name, year) {
  const set = moeExcellentCertifiedSetByYear.get(year);
  if (!set) return null;
  return set.has(normalizeSchoolName(name)) ? "우수" : null;
}
